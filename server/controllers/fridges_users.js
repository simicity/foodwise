...
const addUserToFridgeByEmail = async (req, res) => {
    try {
        ...
        const manager = await pool.query('SELECT user_id FROM fridges WHERE id = $1', [fridge_id])
        if ((!req.user) || manager.rows[0].user_id !== req.user.id) {
            res.status(409).json( { error: `User ${req.user.id} is not the manager of fridge ${fridge_id}` } )
            return
        }
